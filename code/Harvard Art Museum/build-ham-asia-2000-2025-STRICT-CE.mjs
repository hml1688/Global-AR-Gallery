#!/usr/bin/env node
/* build-ham-asia-2000-2025-STRICT-CE.mjs
 * Harvard Art Museums · Asia · 2000–2025 (CE only)
 * Drop anything that references BC/BCE or resolves to all-negative years.
 */
...
const ASIA_COUNTRIES = [
  "China","Japan","India","Korea","Iran","Turkey","Thailand",
...
/* ----- date parsing (HTML logic, patched) ----- */
function parseYearRange(input){
  ...
  // numeric range (patched: optional "c." before 2nd year)
...
async function buildAsia() {
  const all = [];
  const seen = new Set();
  for (const c of ASIA_COUNTRIES) {
    console.log(`=== ${c} ===`);
  ...
  const fname = "offline-ham-asia-2000-2025-STRICT-CE.json";
  await writeFile(fname, JSON.stringify(dedup, null, 2));
  console.log(`Wrote ${fname}`);
}

buildAsia().catch(e=>{console.error(e);process.exit(1);});
